...
2. **Type safety** — [code]
   ...
   *Current progress:* classic SPA `checkJs`, contract-marker checks, and a strict
   ...
   draft rows; redundant classic payment-route/month and payment lifecycle wrapper helpers have also been removed. SACCO admin member-document rows, retention summaries, statement totals, and
