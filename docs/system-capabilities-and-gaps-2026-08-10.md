...
- Member-facing provider tiles are filtered from backend provider options; M-Pesa remains hidden by
  product decision.
- Bank collection has a member self-service reference/draft flow, but real bank API integration is
  still pending.
...
- **Bank collection depth is partial.** The member reference/draft flow exists, but real bank API
  integration, reconciliation import/statement matching, and bank notification workflows remain.
