...
| Production-readiness bundle | `npm.cmd run ready:check` | Isolated Java/PostgreSQL stack passes Flyway, API smoke, security, UI, browser regression, and browser UAT checks. | Passed on 2026-07-18 |
...
| Load/soak readiness | `npm.cmd run load:readiness` | Hosted load targets, soak targets, scenario coverage, evidence fields, and production blockers remain documented. | Ready for use |
...
| Hosted operations evidence | `npm.cmd run operations:hosted-check` | Alert routing, dashboards, centralized logs, off-server backups, schedules, restore drills, and incident contacts remain documented. | Ready for use |
| Pilot launch decision | `npm.cmd run pilot:launch-check` | Go/no-go owners, evidence inputs, accepted-finding rules, launch-window checks, and blockers remain documented. | Ready for use |
...
| Local release evidence command | `npm.cmd run release:evidence` | Deployment, DR, secrets, database, HA, data-protection, hosted operations evidence, pilot launch decision, provider sandbox readiness, Vite, i18n, accessibility, accessibility audit readiness, and repository hygiene contracts pass and produce a timestamped local release pack. | Ready for use |
...
| P2 | Data migration | Member, profile metadata, opening balance, loan book, and repayment history imports now include CSV/Excel templates, dry-run validation, all-or-nothing Java-backed creation/posting, UI paste flows, import guide coverage, and migration evidence reports; external file/object-storage reconciliation remains operational setup. | Pilot SACCO data imports without manual database edits. |
...
- `npm.cmd run load:readiness` passes and hosted load/soak evidence is attached when the release targets production.
...
- Hosted operations evidence covers alert routing, dashboards, centralized logs, off-server backup schedule, restore drill, RPO/RTO, and incident contacts.
- Pilot launch decision evidence covers go/no-go owners, release inputs, accepted-finding limits, launch-window checks, rollback owner, restore owner, and incident contact.
...
- Accessibility audit readiness evidence covers WCAG 2.1 AA scope, keyboard, screen-reader, contrast, zoom, touch, reduced-motion, assistive-technology coverage, and closure requirements.
