...
| --- | --- | --- | --- |
| P0 | Authorization | Add endpoint-level permission checks for each role, including platform-only, SACCO admin, staff, and member-only boundaries. | Permission tests prove blocked actions return `403` and allowed actions still pass. |
...
1. Endpoint-level permission enforcement across Java controllers.
